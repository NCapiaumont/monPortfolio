...
if (!isset($_COOKIE['sent'])){//verification avec un cookie que le mail n'est pas envoyé plusieurs fois (avec un rafrechissement de page)
	...
	if (mail($to, $subject, $msg, $headers)){
		...
		echo ("<h1>Votre demande de nouvelle machine pour <i>".$emailAdmin."</i> a bien été prise en compte.</h1>
<p>Merci d'avoir utiliser l'interface de demande de machines</p>
<p>Le fichier de conf <strong>".$nomFichier.$extension."</strong> a était généré ! Vous pouvez le consulter dans le dossier : <a href='conf'>conf</a></p>");
		...
		*/
		...
		echo ("<h1>Erreur lors de l'envoi de l'email !</h1>");
	}
}
/* Cas où le cookie est créé est qu'il est donc impossible d'envoyer un nouvel email */
else{
		echo ("<h1>Vous ne pouvez pas encore faire de nouvelle demande !</h1><p>Vous pouvez envoyer une nouvelle demande toutes les 2 minutes au minimum.</p><p>Merci de refaire une demande ultérieurement.");
}
?>
